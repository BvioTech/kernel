...
static const struct of_device_id hyn_of_match_table[] = {
	{ .compatible = "hyn,cst3640", .data = &cst66xx_fuc },
    {},
};
...
#endif
{
    ...
    ts_data->fb_notif.notifier_call = fb_notifier_callback;
    HYN_INFO("Rockchip panel notifier register");
    ret = devm_rockchip_panel_notifier_register_client(ts_data->dev,
                                                        &ts_data->fb_notif);
    if(ret < 0){
        HYN_ERROR("Rockchip panel notifier register failed: %d", ret);
    }
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
#endif
#elif defined(CONFIG_DRM)
	/* Registered with devm; the device core unregisters it exactly once. */
#elif defined(CONFIG_HAS_EARLYSUSPEND)

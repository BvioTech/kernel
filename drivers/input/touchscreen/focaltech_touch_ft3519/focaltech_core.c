...
/*
 * Apply the standard device-tree touchscreen orientation before reporting
 * finger coordinates.  The vendor FT3519 driver predates the generic
 * touchscreen helpers, so it has to consume these two properties itself.
 */
static void fts_transform_touch_events(struct fts_ts_data *ts_data,
                                       struct ts_event *events)
{
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        if (pdata->invert_x)
            events[i].x = pdata->x_min + pdata->x_max - events[i].x;
        if (pdata->invert_y)
            events[i].y = pdata->y_min + pdata->y_max - events[i].y;
    }
}
...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    fts_transform_touch_events(ts_data, events);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    pdata->invert_x = of_property_read_bool(np, "touchscreen-inverted-x");
    pdata->invert_y = of_property_read_bool(np, "touchscreen-inverted-y");
    FTS_INFO("coordinate transform: invert_x=%d invert_y=%d",
             pdata->invert_x, pdata->invert_y);
...
static int fts_notifier_callback_init(struct fts_ts_data *ts_data)
{
    ...
    ret = devm_rockchip_panel_notifier_register_client(ts_data->dev,
                                                        &ts_data->fb_notif);
    if (ret)
        FTS_ERROR("register Rockchip panel notifier failed: %d", ret);
...
{
    FTS_FUNC_ENTER();
    devm_rockchip_panel_notifier_unregister_client(ts_data->dev);
    FTS_FUNC_EXIT();
    return 0;
}

...
/*
 * Violoop: ODM 的 drop 是树外扁平目录，故写成 `#include "pcie-dw-dmatest.c"` 且
 * 用 `#if 1` 无条件包含。树内恢复上游守卫——它正是为 =m 这种组合设计的：
 * dmatest.c 没有任何 EXPORT_SYMBOL，内建那份的符号模块链不到，
 * 所以 RKEP 建成模块时必须把 .c 文本包含进来自带一份。
 */
...
/*
 * Violoop: 第一个参数是**设备总线类型**，必须是 pci。上游（含 ODM drop）写的是
 * 驱动名 pcie_rkep，modpost 认不出这个类型，就不生成
 * `pci:v00001D87d0000182A...` 的 modalias → 模块**永远不会被 udev 自动加载**
 * （ODM 的 rknn3_startup 只能按绝对路径 insmod，根因就在这里）。
 * 改成 pci 后：算力卡在位才加载，不在位完全惰性——这正是选配硬件要的行为。
 */

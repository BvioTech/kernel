...
/*
 * Some boards feed a differential mono PA from the stereo LOUT2/ROUT2 pins.
 * Keep the vendor stereo routing intact and invert only the right DAC on
 * those boards.  This is opt-in so the generic ES8389/ES8390 behaviour and
 * other boards remain unchanged.
 */
static void es8389_apply_output_polarity(struct es8389_private *es8389)
{
	if (!es8389->invert_right_output)
		return;

	regmap_update_bits(es8389->regmap, ES8389_DAC_INV_REG45,
			   BIT(6) | BIT(5), BIT(6));
}

static void es8389_apply_initial_volume(struct es8389_private *es8389)
{
	if (!es8389->dac_volume_configured)
		return;

	regmap_write(es8389->regmap, ES8389_DACL_VOL_REG46,
		     es8389->dac_volume);
...
/*
 * TL V4 feeds the differential MIX2052 speaker output back into ES8390
 * LIN2/RIN2 through the two 2 uF AC-coupling capacitors.  This control
 * switches both PGA inputs together; the two original vendor controls stay
 * available for low-level diagnostics.
 */
static int es8389_apply_capture_source(struct es8389_private *es8389)
{
	unsigned int pga1 = ES8389_PGA_DIFFERENTIAL;
	unsigned int pga2 = ES8389_PGA_DIFFERENTIAL;
	int ret;

	if (es8389->speaker_feedback_capture) {
		pga1 = ES8389_PGA1_LINE2P;
		pga2 = ES8389_PGA2_LINE2N;
	}
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = es8389_apply_capture_source(es8389);
	if (ret) {
		es8389->speaker_feedback_capture = previous;
		return ret;
	}
...
static const struct snd_kcontrol_new es8389_snd_controls[] = {
	...
	/*
	 * ES8388-compatible alias.  Violoop userspace addresses the digital DAC
	 * volume as a single stereo control named "PCM"
	 * (audio.playbackVolumeControl defaults to "PCM", documented there as
	 * the ES8388 control).  ES8389/ES8390 splits that volume into the
	 * per-channel DACL/DACR registers, so publish one stereo control over
	 * both of them.  Purely additive: the per-channel controls above and
	 * every register default stay untouched.
	 */
	SOC_DOUBLE_R_TLV("PCM Playback Volume", ES8389_DACL_VOL_REG46,
			ES8389_DACR_VOL_REG47, 0, 0xFF, 0, dac_vol_tlv),
	...
	SOC_SINGLE("ADCL CF TO ADCR", ES8389_ADC_MODE_REG23, 4, 1, 0),
	SOC_SINGLE("ADCR CF TO ADCL", ES8389_ADC_MODE_REG23, 5, 1, 0),
};
...
static void es8389_init(struct snd_soc_component *codec)
{
	...
	es8389_apply_capture_source(es8389);
...
static int es8389_probe(struct snd_soc_component *codec)
{
	...
	dev_info(codec->dev, "speaker feedback capture %s\n",
		 es8389->speaker_feedback_capture ? "enabled" : "disabled");
	if (!device_property_read_u32(codec->dev,
				      "everest,dac-playback-volume", &volume)) {
		es8389->dac_volume_configured = true;
		es8389->dac_volume = min_t(u32, volume, 0xff);
	}
...
static void es8389_remove(struct snd_soc_component *codec)
{
	...
	usleep_range(500000, 550000); /* 500 ms */
